...
fun solution(wallpaper: Array<String>): IntArray {
    // 파일('#')이 존재하는 최소/최대 행과 열의 인덱스를 구합니다.
    var minRow = Int.MAX_VALUE
    var minCol = Int.MAX_VALUE
    var maxRow = 0
    var maxCol = 0

    // wallpaper의 각 행과 열을 순회하며 '#'인 위치의 인덱스를 갱신합니다.
    ...
    // 드래그할 때 선택되는 영역은
    // 시작점 (lux, luy) = (minRow, minCol)와
    // 끝점 (rdx, rdy) = (maxRow + 1, maxCol + 1) 입니다.
    return intArrayOf(minRow, minCol, maxRow + 1, maxCol + 1)
}

// 아래는 테스트를 위한 main 함수입니다.
fun main() {
    val testCases = listOf(
        // 예제 1
        Pair(arrayOf(".#...", "..#..", "...#."), intArrayOf(0, 1, 3, 4)),
        // 예제 2: 한 파일만 있는 경우
        Pair(arrayOf("..........", ".....#....", ".........."), intArrayOf(1, 5, 2, 6)),
        // 예제 3: 좌측 하단에 파일이 있는 경우
        Pair(arrayOf("..", "#."), intArrayOf(1, 0, 2, 1))
    )

    for ((input, expected) in testCases) {
        val result = solution(input)
        println("Input: ${input.joinToString(separator = "\n")}")
        println("Expected: ${expected.joinToString()}")
        println("Result: ${result.joinToString()}")
        println("Test passed: ${result.contentEquals(expected)}")
        println("------")
    }
}